...
void		print_top(const char *content, t_term *term)
{
	...
	ft_putnbr_fd(term->qty_s, 2);
	ft_putstr_fd("  Page : ", 2);
	ft_putnbr_fd(term->page, 2);
	ft_putstr_fd("/", 2);
	ft_putnbr_fd(term->max_p, 2);
	ft_putstr_fd("  ", 2);
	print_cap("me");
}
...
void		print_bottom(t_term *term)
{
	...
	ft_putstr_fd("\x1B[7m[SPACE]\x1B[0m   Select/Unselect  ", 2);
	ft_putstr_fd("\x1B[7m[M-Left]\x1B[0m   Previous Page       ", 2);
	ft_putstr_fd("\x1B[7m[M-Right]\x1B[0m  Next Page        ", 2);
	goto_cap("cm", 0, term->wsize.ws_row - 1);
	ft_putstr_fd("\x1B[7m[RETURN]\x1B[0m  Valid Selection  ", 2);
	ft_putstr_fd("\x1B[7m[ESC]\x1B[0m      Quit without saving", 2);
}

void		print_bottom_sm(t_term *term)
{
	goto_cap("cm", 0, term->wsize.ws_row - 2);
	ft_putstr_fd("\x1B[7m[SPA]\x1B[0m  Select/Unselect ", 2);
	ft_putstr_fd("\x1B[7m[M-L]\x1B[0m  Prev.  ", 2);
	ft_putstr_fd("\x1B[7m[M-R]\x1B[0m  Next", 2);
	goto_cap("cm", 0, term->wsize.ws_row - 1);
	ft_putstr_fd("\x1B[7m[RET]\x1B[0m  Valid           ", 2);
	ft_putstr_fd("\x1B[7m[ESC]\x1B[0m  Quit ", 2);
}
