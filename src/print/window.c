...
{
	box_top(1, 0, term->wsize.ws_col - 4, term->fd);
	box_left_or_right(1, 1, term->fd);
	box_left_or_right(term->wsize.ws_col - 2, 1, term->fd);
	box_bottom(1, 2, term->wsize.ws_col - 4, term->fd);
	goto_cap("cm", 3, 1);
...
void		print_bottom(t_term *term)
{
	...
	ft_putstr_fd("\x1B[7m[M-Left]\x1B[0m   Previous Page       ", term->fd);
	ft_putstr_fd("\x1B[7m[M-Right]\x1B[0m  Next Page        ", term->fd);
...
void		print_bottom_sm(t_term *term)
{
	...
	ft_putstr_fd("\x1B[7m[M-L]\x1B[0m  Prev.  ", term->fd);
	ft_putstr_fd("\x1B[7m[M-R]\x1B[0m  Next", term->fd);
	goto_cap("cm", 0, term->wsize.ws_row - 1);
